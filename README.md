# TV Auto

TV Auto 是一款面向 Android TV、手机和平板的轻量级电视直播浏览器。它不依赖传统 IPTV 源列表，而是把网络直播网页整理成频道，让用户用接近电视的方式浏览、切换和管理网页直播内容。

当前版本：**v6.0**  
包名：`mulin.tvauto.pro`  
最低系统：**Android 4.4**

---

## 适合谁使用

TV Auto 适合这几类用户：

- 想把常看的直播网页整理成频道，打开软件就直接看
- 希望电视端尽量简单，把输入、导入、整理这些操作放到手机上完成
- 遇到特殊站点时，愿意用自定义脚本做进一步适配的高级用户

TV Auto 的基本思路不是维护传统 IPTV 源，而是尽量接管用户访问的直播网页。这样做更自由，但也意味着：如果网站本身要求登录、强制交互、限制播放，或页面结构后来发生变化，TV Auto 也不能保证它永远可用。

---

## 主要功能

- 内置默认频道，首次安装即可直接使用
- 支持遥控器、键盘和触摸屏操作
- 支持扫码或输入局域网地址进入网页管理页，电视负责播放，手机、平板或电脑负责整理
- 支持频道新增、删除、恢复默认、清空、批量导入和导出
- 支持用户脚本：可为特定站点添加专属适配，并单独导入或导出
- 支持原始网页模式：遇到登录、授权或特殊页面时，可临时退出播放接管
- 加载时显示半透明 TVAuto 提示层，弱网或断网时仍能看到页面状态

---

## 日常使用

### 正常播放

打开后会直接进入频道播放界面。

![正常播放](docs/images/normal-play.jpg)

### 切换频道

- 上 / 下键：切换上一频道 / 下一频道
- 数字键：直接输入频道号
- OK / 右键：打开右侧频道列表

![频道列表](docs/images/channel-list.jpg)

### 频道管理

按遥控器 **菜单键** 打开频道管理。电视端可以手动新增单个频道、删除当前频道、检测更新，也可以把更适合输入设备完成的整理工作交给浏览器管理页。

![频道管理扫码](docs/images/channel-management-qr.jpg)

手机、平板或电脑与电视处于同一局域网时，都可以通过扫码或直接输入二维码下方地址进入网页管理页。这个服务只在频道管理窗口打开期间启用，关闭窗口后会自动停止。

![网页管理频道列表](docs/images/mobile-channel-management.jpg)

网页管理页支持：

- 新增、删除、恢复默认和清空频道
- 批量导入频道
- 导出当前频道列表为 `.txt`
- 检测更新
- 管理用户脚本

---

## 频道导入格式

频道导入和导出均使用纯文本 `.txt`，每个频道以英文分号结尾：

```txt
"频道名称","频道地址";
```

示例：

```txt
"CCTV-1 综合","https://tv.cctv.com/live/cctv1/";"江苏卫视","https://live.jstv.com/";
```

导出的文件名会自动带上实际日期，便于备份和区分版本。

完整示例可参考：[`docs/examples/channels.txt`](docs/examples/channels.txt)

---

## 用户脚本

默认脚本已经可以处理一批常见页面，但它不是万能的。某些站点的页面结构比较特殊，默认脚本可能无法得到理想的全屏效果。

例如下面这个页面，默认脚本无法完全接管：

![默认脚本不能全屏](docs/images/default-script-limitation.jpg)

这时，高级用户可以在网页管理页为该站点添加专属脚本。站点可以填写完整 URL，也可以填写域名或 URL 片段；当前页面命中后，会优先执行用户脚本，而不是默认脚本。

![网页管理用户脚本区](docs/images/mobile-user-script-empty.jpg)

![添加用户脚本](docs/images/mobile-user-script-add.jpg)

![添加后的用户脚本](docs/images/mobile-user-script-added.jpg)

添加脚本后，同一站点就可以按用户自己的规则重新适配：

![使用用户脚本后的效果](docs/images/custom-script-result.jpg)

用户脚本与频道列表独立保存，可单独导入或导出。示例脚本可参考：[`docs/examples/shenzhen-tv-script.js`](docs/examples/shenzhen-tv-script.js)

---

## 遇到需要登录的网站

有些网站本身要求登录后才能观看。TV Auto 不会也不应该尝试绕过登录，这时可以进入 **原始网页模式**，临时回到网站自己的页面完成必要操作。

![频道需要登录](docs/images/login-required.jpg)

进入方式：

- 全屏播放时连续按两次左键
- 或在频道管理窗口中点击 `原始网页`

进入后，页面顶部会常驻提示：`当前为原始网页，按返回回到播放`

![进入原始网页模式](docs/images/raw-webpage-mode.jpg)

> 原始网页模式更适合触屏或接入鼠标后使用。复杂网页通常不适合只靠遥控器完成操作。

---

## 遥控器操作

| 按键 | 功能 |
| --- | --- |
| 上 / 下 | 切换上一频道 / 下一频道 |
| OK / 右 | 打开右侧频道列表 |
| 左 | 连续按两次进入原始网页模式 |
| 菜单 | 打开频道管理 |
| 数字键 | 直接输入频道号 |
| 返回 | 播放模式下双击退出；原始网页模式下返回播放 |

---

## 使用须知

- 默认频道仅作示例用途，不保证长期可用，也不保证适用于所有地区
- 请用户自行核实并添加合规频道
- TV Auto 只负责整理和展示用户访问的网页内容，不提供直播内容本身
- 使用过程中请遵守当地法律法规，开发者不承担因用户自行添加内容而产生的法律责任

---

## v6.0 更新

- 新增扫码或局域网地址进入网页管理页
- 新增网页管理端频道新增、删除、恢复默认、清空、批量导入与导出
- 新增网页管理端更新检测
- 新增用户脚本管理、导入和导出
- 新增原始网页模式，便于处理登录、授权和特殊站点
- 默认频道改为内置，首次安装无需手动导入
- 优化频道列表在不同设备上的显示密度
- 默认加载提示改为半透明显示，弱网时仍可查看页面状态

---

## 历史版本

### v5.0
- 全新设计界面

### v3.2
- 新增 Android TV 图标

### v3.1
- 修复深色模式
- 新增更新按键

### v3.0
- 新增长按屏幕隐藏 / 显示侧边按键
- 新增频道管理按钮
- 修复全屏播放时虚拟按键和导航条无法隐藏的问题

---

## 反馈与支持

如果你在使用中遇到问题，欢迎反馈。  
如果你觉得 TV Auto 对你有帮助，也欢迎支持作者继续维护。

<div align="center">
  <img src="https://github.com/user-attachments/assets/d6cde049-064a-4fe2-96cb-d66c67c9c21a" width="220" alt="赞赏支持">
</div>
