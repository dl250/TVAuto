# TV Auto

TV Auto 是一个面向 Android TV、手机和平板的网页直播播放器。它不维护传统 IPTV 源，而是把网络直播网页整理成可切换、可管理、可扩展的频道，让“打开网页看直播”这件事更接近真正的电视体验。

当前版本：**v6.0**  
需要兼容老旧系统 WebView 的设备，可查看 [TV Auto X5 分支](https://github.com/ymlyyy/TVAuto/tree/x5)。

---

## 项目思路

TV Auto 的核心不是“收集一份永远有效的源”，而是把网络上的直播页面抽象成频道：

- 对常见页面，使用默认脚本接管视频区域并提供沉浸式播放
- 对特殊页面，允许为站点添加专属用户脚本
- 对必须登录或需要人工操作的页面，提供原始网页模式作为回退
- 对不适合在电视端完成的输入和整理，提供局域网页面统一管理

这种方案比传统源列表更自由，也更贴近真实网页生态；代价是站点本身的登录策略、页面结构和播放器实现仍然会影响最终效果。

---

## 核心能力

- 内置默认频道，安装后即可直接使用
- 支持遥控器、键盘和触摸屏
- 支持扫码或输入局域网地址进入网页管理页
- 支持频道新增、删除、恢复默认、清空、批量导入和导出
- 支持用户脚本，为不同站点提供独立适配能力
- 支持原始网页模式，便于处理登录、授权和特殊交互
- 加载提示采用半透明覆盖，弱网或断网时仍能保留页面上下文

---

## 使用方式

### 播放与切台

打开后会直接进入频道播放界面。

![正常播放](docs/images/normal-play.jpg)

- 上 / 下键：切换频道
- 数字键：直接输入频道号
- OK / 右键：打开右侧频道列表

![频道列表](docs/images/channel-list.jpg)

### 频道与脚本管理

按遥控器 **菜单键** 打开频道管理。电视端保留最常用的入口，复杂输入交给浏览器管理页完成。

![频道管理扫码](docs/images/channel-management-qr.jpg)

手机、平板或电脑与电视处于同一局域网时，都可以通过扫码或直接输入二维码下方地址进入网页管理页。该服务只在频道管理窗口打开期间启用，关闭窗口后会自动停止。

![网页管理频道列表](docs/images/mobile-channel-management.jpg)

网页管理页支持：

- 新增、删除、恢复默认和清空频道
- 批量导入频道
- 导出当前频道列表为 `.txt`
- 检测更新
- 管理、导入和导出用户脚本

---

## 用户脚本

默认脚本已经可以处理一批常见页面，但它并不试图覆盖所有站点。遇到结构特殊的页面，可以直接为该站点添加专属脚本。

下面这个页面中，默认脚本无法得到理想的接管效果：

![默认脚本不能全屏](docs/images/default-script-limitation.jpg)

在网页管理页中添加脚本时，站点既可以填写完整 URL，也可以填写域名或 URL 片段；当前页面命中后，会优先执行用户脚本，而不是默认脚本。

![网页管理用户脚本区](docs/images/mobile-user-script-empty.jpg)

![添加用户脚本](docs/images/mobile-user-script-add.jpg)

![添加后的用户脚本](docs/images/mobile-user-script-added.jpg)

脚本生效后，同一站点可以按自己的规则重新适配：

![使用用户脚本后的效果](docs/images/custom-script-result.jpg)

用户脚本与频道列表独立保存，可单独导入或导出。示例脚本见 [`docs/examples/shenzhen-tv-script.js`](docs/examples/shenzhen-tv-script.js)。

---

## 原始网页模式

有些网站要求登录后才能观看。TV Auto 不尝试绕过站点自身规则，而是提供原始网页模式，让页面在需要时回到网站本来的样子。

![频道需要登录](docs/images/login-required.jpg)

进入方式：

- 全屏播放时连续按两次左键
- 或在频道管理窗口中点击 `原始网页`

进入后，页面顶部会常驻提示：`当前为原始网页，按返回回到播放`

![进入原始网页模式](docs/images/raw-webpage-mode.jpg)

> 原始网页模式更适合触屏或接入鼠标后使用。复杂网页通常不适合只靠遥控器完成操作。

---

## 数据格式

频道导入和导出均使用纯文本 `.txt`，每个频道以英文分号结尾：

```txt
"频道名称","频道地址";
```

示例：

```txt
"CCTV-1 综合","https://tv.cctv.com/live/cctv1/";"江苏卫视","https://live.jstv.com/";
```

导出的文件名会自动带上实际日期，便于备份和区分版本。完整示例见 [`docs/examples/channels.txt`](docs/examples/channels.txt)。

---

## 遥控器操作

| 按键 | 功能 |
| --- | --- |
| 上 / 下 | 切换上一频道 / 下一频道 |
| OK / 右 | 打开右侧频道列表 |
| 左 | 连续按两次进入原始网页模式 |
| 菜单 | 打开频道管理 |
| 数字键 | 直接输入频道号 |
| 返回 | 播放模式下双击退出；原始网页模式下返回播放 |

---

## v6.0 更新

- 新增局域网页面管理，可通过扫码或地址进入
- 新增频道新增、删除、恢复默认、清空、批量导入与导出
- 新增更新检测入口
- 新增用户脚本管理、导入和导出
- 新增原始网页模式
- 默认频道改为内置
- 优化频道列表在不同设备上的显示密度
- 加载提示改为半透明显示

---

## 使用须知

- 默认频道仅作示例用途，不保证长期可用，也不保证适用于所有地区
- 请自行核实并添加合规频道
- TV Auto 只负责整理和展示用户访问的网页内容，不提供直播内容本身
- 使用过程中请遵守当地法律法规

---

## 历史版本

### v5.0
- 全新设计界面

### v3.2
- 新增 Android TV 图标

### v3.1
- 修复深色模式
- 新增更新按键

### v3.0
- 新增长按屏幕隐藏 / 显示侧边按键
- 新增频道管理按钮
- 修复全屏播放时虚拟按键和导航条无法隐藏的问题

---

## 反馈与支持

如果你在使用中遇到问题，欢迎反馈。  
如果你觉得 TV Auto 对你有帮助，也欢迎支持作者继续维护。

<div align="center">
  <img src="https://github.com/user-attachments/assets/d6cde049-064a-4fe2-96cb-d66c67c9c21a" width="220" alt="赞赏支持">
</div>
